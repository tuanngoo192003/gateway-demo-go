...
func (rq *RoleQuery) loadAccounts(ctx context.Context, query *AccountQuery, nodes []*Role, init func(*Role), assign func(*Role, *Account)) error {
	...
	for _, n := range neighbors {
		fk := n.role_accounts
		if fk == nil {
			return fmt.Errorf(`foreign-key "role_accounts" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "role_accounts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
