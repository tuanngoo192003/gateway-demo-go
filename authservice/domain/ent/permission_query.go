...
func (pq *PermissionQuery) QueryPermissions() *PermissionQuery {
...
func (pq *PermissionQuery) WithPermissions(opts ...func(*PermissionQuery)) *PermissionQuery {
	query := (&PermissionClient{config: pq.config}).Query()
...
func (pq *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := pq.withPermissions; query != nil {
		if err := pq.loadPermissions(ctx, query, nodes,
			func(n *Permission) { n.Edges.Permissions = []*Permission{} },
			func(n *Permission, e *Permission) { n.Edges.Permissions = append(n.Edges.Permissions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PermissionQuery) loadPermissions(ctx context.Context, query *PermissionQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *Permission)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(permission.PermissionsTable)
		s.Join(joinT).On(s.C(permission.FieldID), joinT.C(permission.PermissionsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(permission.PermissionsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(permission.PermissionsPrimaryKey[0]))
