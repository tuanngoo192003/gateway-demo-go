...
func getPath() string {
	// Get the current working directory (which will be inside cmd/)
	dir, err := os.Getwd()
	if err != nil {
		panic(err)
	}
	...
	// Construct path to logs directory at project root
	...
	return filepath.Join(logDir, getEnv("LOG_FILE_PATH", ""))
}
